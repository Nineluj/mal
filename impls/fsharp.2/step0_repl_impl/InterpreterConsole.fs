...
let pushHistory (history: CommandHistory) (newCommand: string) =
    let rec innerFn (ch: CommandHistory) =
        ...
        | [] -> ch
        | h :: t -> innerFn { historyFuture = t; historyPast = h::ch.historyPast }
    let restoredHistory = innerFn history
    { restoredHistory with historyPast = newCommand::restoredHistory.historyPast }
...
let readBasic () =
    SubmitCommandBasic (Console.ReadLine())

let updateState (state: ConsoleState) (action: UserActionType): ConsoleState * CommandResult =
    match action with
    | CharacterInput c -> { state with input = { text = state.input.text + string c } }, None
    | SubmitCommand ->
        { input = defaultInput; history = pushHistory state.history state.input.text }, Execute state.input.text
    | DeleteLastChar ->
        let newText = match state.input.text, state.input.text.Length with
                        | s, 0 -> s
                        | s, sLength -> s[..sLength-2]
        { state with input = { text = newText } }, None
    | HistoryBack ->
        match (state.history.historyPast, state.history.historyFuture) with
        | [], _ -> { state with input = defaultInput }, None
        | h :: t, future -> { input = { text = h }; history = { historyFuture = h::future; historyPast = t } }, None
    | HistoryForward ->
        match (state.history.historyPast, state.history.historyFuture) with
        | _, [] -> { state with input = defaultInput }, None
        | past, h :: t -> { input = { text = h }; history = { historyFuture = t; historyPast = h::past } }, None
    | ClearInput -> { state with input = defaultInput }, None
    | SubmitCommandBasic s -> state, Execute s

let renderBasic (_: ConsoleState): unit =
    Console.Write "user> "
...
let render (state: ConsoleState): unit =
    ...
    | n when n > 0 ->
        Console.Write(String.replicate n "\b \b")
    | _ -> ()
...
let renderResultBasic result =
    match result with
    | Execute txt ->
        REPL.rep txt |> sprintf "%s\n" |> Console.Write
    | None -> ()

let renderResult result =
    match result with
    | Execute txt ->
        REPL.rep txt |> sprintf "\n%s\n" |> Console.Write
    | None -> ()

let run isInteractive =
    let render, read, renderResult =
        if isInteractive then
            render, readOne, renderResult
        else
            renderBasic, readBasic, renderResultBasic
    let mutable state = { input = defaultInput; history = defaultCommandHistory }
    while true do
        render state
